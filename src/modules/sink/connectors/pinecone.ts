...
class PineconeConnector implements SinkConnector {
  ...
  async upsert(records: VectorRecord[], namespace?: string): Promise<UpsertResult> {
    ...
    const vectorBytes = sample.values.length * 4;
    const metadataBytes = sample.metadata ? Buffer.byteLength(JSON.stringify(sample.metadata), "utf8") : 0;
    const bytesPerRecord = vectorBytes + metadataBytes + 200;
